...
public class StartMatchmakingRequest : Tdf
{
    static readonly TdfMemberInfo[] __typeInfos = [
        ...
        new TdfMemberInfo("GameProtocolVersionString", "mGameProtocolVersionString", 0x9F697200, TdfType.String, 7, true), // GVER
        new TdfMemberInfo("IgnoreEntryCriteriaWithInvite", "mIgnoreEntryCriteriaWithInvite", 0xA67BAF00, TdfType.Bool, 8, true), // IGNO
        new TdfMemberInfo("SessionMode", "mSessionMode", 0xB6F92500, TdfType.Enum, 9, true), // MODE
        new TdfMemberInfo("NetworkTopology", "mNetworkTopology", 0xBB4BF000, TdfType.Enum, 10, true), // NTOP
        new TdfMemberInfo("PlayerAttribs", "mPlayerAttribs", 0xC21D3400, TdfType.Map, 11, true), // PATT
        new TdfMemberInfo("PlayerIdList", "mPlayerIdList", 0xC2CCF400, TdfType.List, 12, true), // PLST
        new TdfMemberInfo("MaxPlayerCapacity", "mMaxPlayerCapacity", 0xC2D87800, TdfType.UInt16, 13, true), // PMAX
        new TdfMemberInfo("PlayerNetworkAddress", "mPlayerNetworkAddress", 0xC2E97400, TdfType.Union, 14, true), // PNET
        new TdfMemberInfo("QueueCapacity", "mQueueCapacity", 0xC6387000, TdfType.UInt16, 15, true), // QCAP
        new TdfMemberInfo("SessionIdList", "mSessionIdList", 0xCE992C00, TdfType.List, 16, true), // SIDL
        new TdfMemberInfo("VoipNetwork", "mVoipNetwork", 0xDAFA7000, TdfType.Enum, 17, true), // VOIP
    ];
    ...
    private TdfString _gameProtocolVersionString = new(__typeInfos[7]);
    private TdfBool _ignoreEntryCriteriaWithInvite = new(__typeInfos[8]);
    private TdfEnum<Blaze2SDK.Blaze.GameManager.MatchmakingSessionMode> _sessionMode = new(__typeInfos[9]);
    private TdfEnum<Blaze2SDK.Blaze.GameNetworkTopology> _networkTopology = new(__typeInfos[10]);
    private TdfMap<string, string> _playerAttribs = new(__typeInfos[11]);
    private TdfList<uint> _playerIdList = new(__typeInfos[12]);
    private TdfUInt16 _maxPlayerCapacity = new(__typeInfos[13]);
    private TdfUnion<Blaze2SDK.Blaze.NetworkAddress> _playerNetworkAddress = new(__typeInfos[14]);
    private TdfUInt16 _queueCapacity = new(__typeInfos[15]);
    private TdfList<uint> _sessionIdList = new(__typeInfos[16]);
    private TdfEnum<Blaze2SDK.Blaze.VoipTopology> _voipNetwork = new(__typeInfos[17]);
    ...
    public Blaze2SDK.Blaze.GameManager.MatchmakingCriteriaData? CriteriaData
    {
    ...
    public Blaze2SDK.Blaze.GameManager.MatchmakingSessionMode SessionMode
    {
